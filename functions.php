...
// === OPTIMIERTER NAVIGATION WALKER (KORRIGIERT) ===

class Luvex_Nav_Walker extends Walker_Nav_Menu {
    
    // Start Level - fügt Sub-Menu Container hinzu
    ...
    // Start Element - erweiterte Logik für 3-Level System
    public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
        ...
        // Prüfe ob Item Kinder hat
        $has_children = in_array('menu-item-has-children', $classes);

        /**
         * Filter hook für zusätzliche CSS-Klassen
         * Ermöglicht es anderen Plugins, Klassen hinzuzufügen
         */
        ...
        // Link-Attribute aufbauen
        $attributes = ! empty($item->attr_title) ? ' title="'  . esc_attr($item->attr_title) .'"' : '';
        $attributes .= ! empty($item->target)     ? ' target="' . esc_attr($item->target     ) .'"' : '';
        $attributes .= ! empty($item->xfn)        ? ' rel="'    . esc_attr($item->xfn        ) .'"' : '';
        $attributes .= ! empty($item->url)        ? ' href="'   . esc_attr($item->url        ) .'"' : '';

        // Link-Content mit intelligenter Pfeil-Logik
        $item_output = isset($args->before) ? $args->before : '';
        $item_output .= '<a' . $attributes . '>';
        $item_output .= (isset($args->link_before) ? $args->link_before : '') . apply_filters('the_title', $item->title, $item->ID) . (isset($args->link_after) ? $args->link_after : '');
        
        // Pfeil hinzufügen basierend auf Depth und Children
        if ($has_children) {
            if ($depth == 0) {
                // Level 1: Nach-unten Pfeil
                $item_output .= ' <i class="fa-solid fa-chevron-down dropdown-arrow"></i>';
            } elseif ($depth == 1) {
                // Level 2: Nach-rechts Pfeil (für vertikale Level 3)
                $item_output .= ' <i class="fa-solid fa-chevron-right dropdown-arrow"></i>';
            }
        }
...
function luvex_enqueue_assets() {
    ...
    $scripts_to_enqueue = [
        ...
        'luvex-mobile-menu' => '/assets/js/mobile-menu.js', // WICHTIG: Wird weiterhin geladen
